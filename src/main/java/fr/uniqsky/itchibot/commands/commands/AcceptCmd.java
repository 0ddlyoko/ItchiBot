...
public class AcceptCmd implements Command, DiscordUtil {

	@Override
	public String getCmd() {
		return ItchiBot.get().getConfigManager().getAcceptCommand();
	}
	...
	public boolean onCommand(String label, String[] args, MessageReceivedEvent e) {
		...
		// Send new message
		try {
			TextChannel tc = getTextChannel(ItchiBot.get().getConfigManager().getAcceptChannel());
			...
			Bukkit.getLogger().log(Level.SEVERE,
					"No permission to read in channel " + ItchiBot.get().getConfigManager().getAcceptChannel(), ex);
		} catch (ErrorResponseException ex) {
		} catch (Exception ex) {
			Bukkit.getLogger().log(Level.WARNING,
					"Exception while accept / write in channel " + ItchiBot.get().getConfigManager().getAcceptChannel(),
					ex);
		}
		return true;
	}
